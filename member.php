...
<div id="about_1">
  <?php echo "<form method='get' name='delete_form' action='delete.php'>";
          echo '<input type="hidden" value='.htmlspecialchars($member).' name="member"/>';
   ?>
  <?php
        ...
        $records = exec_sql_query($db, $sql)->fetchAll();
          echo "<li>";
          echo "<h1>" . htmlspecialchars($records[0]['first_name']) . " " . htmlspecialchars($records[0]['last_name']) . "</h1>";
          echo "<img class='team_imgs' src=" . htmlspecialchars($records[0]['picpath']) . ">";
